  ...
  let getfoodDetails = async (event) => {
    try {
      ...
      const { data } = await axios.get(
        ...
        {
          headers: { "X-Api-Key": "sT7HC07pGGiYeQ3vYCmtag==VrV5LeqsENpqDsIQ" },
        }
      );
      console.log(data);
      Setload(false);
    ...
    <div className="container addfood-container">
      <form onSubmit={getfoodDetails}>
        <InputGroup className="searchcon">
          <FormControl
            placeholder="1kg rice with 500g of brinjal"
            name="search"
            value={search}
            onChange={(event) => {
          ...
          ></FormControl>
          <Button className="innerbtn" type="submit">
            <FaSearch />
          </Button>
        </InputGroup>
      </form>
                  ...
                  <div key={index} className="card">
                    <div className="card-body">
                      ...
                      <p className="card-text">
                        <span>Calories (kcal):</span> {f.calories}
                      </p>
                      <p className="card-text">
                        <span>Grams:</span> {f.serving_size_g} g
                      </p>
                      <p className="card-text">
                        <span>Fat: </span>
                        {f.fat_total_g} g
                      ...
                      <p className="card-text">
                        <span>Fiber:</span> {f.fiber_g} g
                      </p>
                      <p className="card-text">
                        <span>Protein:</span> {f.protein_g} g
                      </p>
                      <div className="text-center">
                        <button
