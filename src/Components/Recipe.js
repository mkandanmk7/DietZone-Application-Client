...
const Recipe = () => {
  ...
  let getfoodDetails = async (event) => {
    ...
    const { data } = await axios.get(
      `https://api.edamam.com/api/recipes/v2?q=${search}&app_id=f6703955&app_key=01ec89ed5748bd0263236aabc0e00218&type=public`
    );
    setload(false);
    Setfood(data.hits);
    console.log(data);
              ...
              food.map((x, index) => {
                return (
                  <Card
                    ...
                    onClick={() => {
                      handleShow(
                        x.recipe ? x.recipe.ingredientLines : ["null"],
                        x.recipe ? x.recipe.label : "",
                        x.recipe ? x.recipe.url : "#"
                      );
                    }}
                    className="card cardrecipe"
                  >
                    <Card.Body>
                      <Card.Title>{x.recipe ? x.recipe.label : ""}</Card.Title>
                      <Card.Img
                        variant="bottom"
                        src={x.recipe ? x.recipe.image : ""}
                      ></Card.Img>
                    </Card.Body>
                  </Card>
                );
